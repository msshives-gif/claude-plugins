...
_DEFAULTS = {
    ...
    "report_min_tokens": 0,
    # At/above this, messaging the agent is gated per block_style
    # (default deny_once: a model-facing challenge the model overrides
    # by retrying). 0 disables blocking. Between warn_tokens and here,
    # the guard only warns.
...
def load_config():
    ...
    # hang deny_once exists to prevent. The upper clamp keeps every
    # active latch younger than the state_ttl_days prune floor (1 day),
    # so pruning can never reap a still-satisfied challenge.
    cfg["deny_once_ttl_seconds"] = min(
        max(30, cfg["deny_once_ttl_seconds"]), 86_400)
...
def _latch_path(cfg, session_id, sender, agent_id):
    ...
    # Digest of the (sender, agent) tuple: separator tricks like
    # ("a", "b--c") vs ("a--b", "c"), a sender literally named "root",
    # or path_component collapsing to "unknown" must never let one
    # pair satisfy another pair's challenge (audit finding). The
    # agent-id prefix is for human debuggability only.
    digest = hashlib.sha256(
        json.dumps([sender, agent_id]).encode()).hexdigest()[:24]
    prefix = path_component(agent_id)[:40]
    return os.path.join(d, f"{prefix}-{digest}.json")


def deny_latch_state(cfg, session_id, sender, agent_id, ttl):
    """One of "fresh" (challenge satisfied: pass), "absent"/"expired"
    (a deny is allowed), "invalid" (unreadable/corrupt/future-dated:
    FAIL OPEN — pass without denying, because denying on state we
    cannot read correctly could repeat on every retry; audit
    finding)."""
    try:
    ...
    if not isinstance(latch, dict):
        return "invalid"
    denied_at = latch.get("denied_at", None)
    if not isinstance(denied_at, (int, float)) \
            or isinstance(denied_at, bool):
        return "invalid"
    elapsed = time.time() - denied_at
    if elapsed < 0:
        return "invalid"  # future-dated (clock step): never fail closed
    return "fresh" if elapsed < ttl else "expired"


def write_deny_latch(cfg, session_id, sender, agent_id, current,
                     compactions, expired=False):
    ...
    observer's atomic writes).

    Concurrency: for a NEW latch the create is O_EXCL, so of N racing
    sends exactly one records the challenge (and denies) — the losers
    see FileExistsError, return False, and pass with the warning.
    Replacing an EXPIRED latch uses an atomic replace; two sends
    racing exactly at expiry can both deny — a rare extra challenge,
    not a loop."""
    try:
        p = _latch_path(cfg, session_id, sender, agent_id)
        _private_makedirs(os.path.dirname(p))
        _write_sentinel(cfg)
        payload = json.dumps({"denied_at": time.time(),
                              "tokens": current,
